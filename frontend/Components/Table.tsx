...
const MortgageTable: React.FunctionComponent = () => {
  const {Type, TermLength, Amortization, HomePrice, DownPayment, MortgageAmount} = useContext(GlobalContext)
  const [TableData, setTableData] = useState([])
;
  // Use Effect calls in the data and sets table values
  useEffect(()=> {
    const getMortgageData = async () => {
       const res = await axios.get('http://127.0.0.1:8000/backend/get_mortgage_data/')
      setTableData(res.data)
    }
    getMortgageData()

  }, []);
  const filteredData = TableData.filter((row)=> row.rate_type === Type.toLowerCase() && row.year.toString() === TermLength.toString())
  return (
              ...
              <StyledTableCell component="th" scope="row">
                ...
                <div className="flex">
                  <img
                    ...
                    alt={row.source}
                  />
                  <div className="text-gray-700 sm:text-lg text-md">{row.source}</div>
                </div>
                </div>
              </StyledTableCell>
              ...
              <StyledTableCell align="center">
                <div className=" flex justify-center text-gray-700 md:text-2xl sm:text-xl text-lg">
                  <div className="w-1/3 justify-center border-b-2 border-orange-500">
                    ${CalculateRate(DownPayment, HomePrice, MortgageAmount, Amortization, row.rate)}
                  </div>
                </div>
              </StyledTableCell>
            </StyledTableRow>
